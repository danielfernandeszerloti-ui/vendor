...
import { LayoutDashboard, Building2, FileText, Server, AlertTriangle, Users, Shield, LogOut, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function Sidebar({ user }: { user: Usuario | null }) {
  ...
  return (
    <aside className={cn(
      'h-screen flex flex-col border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 transition-all duration-200 relative flex-shrink-0',
      collapsed ? 'w-16' : 'w-60'
    )}>
      <button onClick={() => setCollapsed(!collapsed)}
        className="absolute -right-3 top-7 z-10 w-6 h-6 rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex items-center justify-center text-gray-400 hover:text-gray-600 shadow-sm">
      ...
      <div className="flex items-center gap-3 h-16 border-b border-gray-200 dark:border-gray-800 px-4 flex-shrink-0"
        ...
        {!collapsed && (
          <div className="overflow-hidden">
            <p className="font-bold text-sm text-white truncate leading-tight">Zerbini do Brasil</p>
            <p className="text-[11px] truncate" style={{ color: '#00c8b4' }}>Gestão de TI</p>
          </div>
        )}
      </div>
        ...
        {!collapsed && user && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800">
            <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, #1a1f6e, #00c8b4)' }}>
              <span className="text-[10px] font-bold text-white">{user.nome.charAt(0).toUpperCase()}</span>
            </div>
            <div className="overflow-hidden">
